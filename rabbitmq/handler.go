...
func HandleRequest(data structs.Request) string {
	...
	case SendMailGmail:
		...
		var count int
		row := config.DB.Raw("SELECT COUNT(*) FROM jobs").Scan(&count)
		if row.Error != nil {
			log.Println("Direct SQL query failed:", row.Error)
		}
		log.Println("Direct SQL query succeeded, job count:", count)

		log.Println("Attempting to send email via Gmail...")
		...
		// err = config.DB.Where("id = ?", data.JobId).First(&job).Error
		// if err != nil {
		// 	log.Printf("Job not found for ID: %s. Error: %v", data.JobId, err)
		// 	return err.Error()
		// }
		// log.Println("Job found for ID:", data.JobId)
